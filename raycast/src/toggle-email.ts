...
/**
 * Same engine as the plain "Toggle Dictation" command, but always passes
 * --format email so this command can be bound to its own hotkey.
 */
...
export default async function Command() {
  ...
  await showHUD(
    stopping
      ? "⏳ Transcribing (email format)…"
      : "🎙️ Recording for email… (run again to stop)",
  );
